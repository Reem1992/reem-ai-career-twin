...
with col1:
    if st.button("Is Curio short?", key="q_curio_short"):
        st.session_state.pending_question = "Is Curio short?"
        st.session_state.messages = []

    if st.button("Tell me about Reem", key="q_about_reem"):
        st.session_state.pending_question = "Tell me about Reem"
        st.session_state.messages = []

    if st.button("What are her strongest achievements?", key="q_achievements"):
        st.session_state.pending_question = "What are her strongest achievements?"
        st.session_state.messages = []

with col2:
    if st.button("How does she use data in her work?", key="q_data_work"):
        st.session_state.pending_question = "How does she use data in her work?"
        st.session_state.messages = []

    if st.button("What type of roles is she targeting?", key="q_roles"):
        st.session_state.pending_question = "What type of roles is she targeting?"
        st.session_state.messages = []

    if st.button("What supply chain experience does she have?", key="q_supply_chain"):
...
# Show only latest saved interaction
